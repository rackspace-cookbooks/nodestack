...
def load_current_resource
  @current_resource = Chef::Resource::NodestackApp.new(new_resource.name)

  if ::File.exist?("#{new_resource.path}/server-start.sh")
    c = cli_run('pm2 list')
...
action :create do
  ...
  server_start = execute "run #{new_resource.name}" do
    ...
    user new_resource.user
    group new_resource.group
    only_if { ::File.exist?("#{new_resource.path}/server-start.sh") }
    action :nothing
  end

  if ! @current_resource.exists || t.updated_by_last_action?
