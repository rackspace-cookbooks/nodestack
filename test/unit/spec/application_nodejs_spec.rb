...
describe 'nodestack::application_nodejs' do
  ...
  OS_OPTS.each do |os,value|
    describe os do
      let(:chef_run) do
        ChefSpec::Runner.new(value) do |node|
          node_resources(node)
      ...
      it "installs package #{value['libcap_package']}" do
        expect(chef_run).to install_package("#{value[:libcap_package]}")
      end
